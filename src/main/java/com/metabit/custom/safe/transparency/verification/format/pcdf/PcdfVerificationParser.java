...
import com.metabit.custom.safe.transparency.verification.VerificationLogger;
...
import com.metabit.custom.safe.transparency.verification.result.Error.Type;
		...
		if (checkTwoBytePublicKeys(dtPK, publicKey)) {
			...
			if (pos != -1) {
				...
				if (!checkSignAndPublicKeyByte(justData, sign, publicKey)) {
					com.metabit.custom.safe.transparency.verification.result.Error er;
					er = new com.metabit.custom.safe.transparency.verification.result.Error(Type.VALIDATION,
			...
			com.metabit.custom.safe.transparency.verification.result.Error er;
			er = new com.metabit.custom.safe.transparency.verification.result.Error(Type.VALIDATION, "Wrong Public Key",
